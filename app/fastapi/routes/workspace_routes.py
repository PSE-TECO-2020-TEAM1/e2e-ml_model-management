...
@router.get("/workspaces/{workspaceId}/trainingState", response_model=TrainingState, status_code=status.HTTP_200_OK)
...
@router.get("/workspaces/{workspaceId}/models/{modelId}", response_model=MlModelInResponse, status_code=status.HTTP_200_OK)
async def get_model(ml_model: MlModel = Depends(get_ml_model_by_id_from_path)):
    return MlModelInResponse(config=parse_config_for_response(ml_model.config),
                             labelPerformanceMetrics=ml_model.label_performance_metrics)
...
async def get_prediction_id(workspaceId: OID, modelId: OID):
    return await generate_prediction_id_for_model(workspaceId, modelId)
